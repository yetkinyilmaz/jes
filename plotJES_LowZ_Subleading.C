...
void plotJES_LowZ_Subleading(double cc = 1, double ss = 0, double nn = 2, string var = "xj", bool doMC = 1, bool doPP = 0){
  ...
  c1->Print(Form("figure_sys_jes_lowzsub_%s_%s_%s_%s_c%d_s%d_n%d.png",var.data(),label.data(),pp?"pp":"PbPb",mc?"mc":"data",int(c*100),abs(int(s)),abs(int(n))));
  ...
  t3->Draw();
  c2->Print(Form("figure_sys_jes_pt_legend_c%d.png",int(c*100)));
