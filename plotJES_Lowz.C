...
void plotJES_Lowz(double nn = 5., string var = "xj", bool doMC = 1, bool doPP = 0){
...
// 6% overall JES uncertainty ???
  ...
  t3->AddEntry(h0,Form("#LTx_{J}#GT: %0.3f",h0->GetMean()),"l");
  t3->AddEntry(h1,Form("#LTx_{J}#GT: %0.3f",h1->GetMean()),"p");
  t3->AddEntry(h2,Form("#LTx_{J}#GT: %0.3f",h2->GetMean()),"p");


  h0->GetXaxis()->SetTitle("x_{J}");
  h0->Draw("hist");
  h1->Draw("same");
  h2->Draw("same");
  //  t3->Draw();
  ...
  t3 = makeLegend(0.67,0.54,0.87,0.9);
  t3->AddEntry(h0,Form("%s",mc?"MC":"Data 0-10%"),"");

  t3->AddEntry(h0,Form("#pm 2 GeV / p_{T}",c),"");
  ...
  t3->Draw();
  c1->Print(Form("figure_sys_jes_lowz_%s_%s_%s_%s_c%d.png",var.data(),label.data(),pp?"pp":"PbPb",mc?"mc":"data",int(c*100)));
